...
let _INFOPNL = false;					// TRUE: lower information panel is loaded.
...
let _NEWCONN = true;					// TRUE: clone of connectivity matrix is created to create modifications. FALSE: only leaves original matrix (doesn't allow modifications).
let _NONLINR = false;					// TRUE: non-linear problem, creates a second set of matrices for non-linear behavior.
let _DEFORMS = false;					// TRUE: deformation is considered in the problem, creates the appropriate matrices.
