...
let nus  = new Array(2).fill(0);																			// Array for the real-time values of nu. Defined as global so it's accesible by all functions.
...
function remesh() {
	...
	AL_PM  = Math.round((slider.alpha - 45) * 99/90);
	...
	for (var i = 0; i < NUM_NODES; i++) {
		for (var j = 0; j < COORS_1[i].length; j++) {
			...
			for (var k = 0; k < cxx.length; k++)
				deltx[i][j] += slider.defx*(uxx[k][i][j]*bxx[k][B_PM]*axx[k][A_PM]*alxx[k][AL_PM]*txx[k][T_PM]*cxx[k]);

			for (var k = 0; k < cyy.length; k++)
				delty[i][j] += slider.defy*(uyy[k][i][j]*byy[k][B_PM]*ayy[k][A_PM]*alyy[k][AL_PM]*tyy[k][T_PM]*cyy[k]);
		}
	}
...
function result() {
	...
	f5p1 = f5.add(slider, 'plot1', ['xx', 'yy']).name('Homogenization case').onChange(function(value) {guiChange(value)});							// Adds the option for modifying the first plot, correspoding to modal amplitudes in all cases.
...
function guiChange(apt) {
	...
	if (apt == 'xx' || apt == 'yy') {
		...
		// If the plot chosen is the modal functions:
			...
			slider.plot3 = 'a';
			f6.add(slider, 'plot3', ['a', 'b', 'alfa', 't']).name('Chosen Parameter').onChange(function() {calcModFunc()});
...
function calcNuVals() {
	...
	XL = COORS_T[0][0], XR = COORS_T[117][0];
	YB = COORS_T[2][1], YT = COORS_T[119][1];
	w0 = XR - XL;
	h0 = YT - YB;
	...
	for (var i = 0; i < NUM_NODES; i++) {
		for (var j = 0; j < deltx[i].length; j++) {
				...
				for (var k = 0; k < cxx.length; k++)
					u_xx[3*i+j] += uxx[k][i][j]*axx[k][A_PM]*bxx[k][B_PM]*alxx[k][AL_PM]*txx[k][T_PM]*cxx[k];
			
				for (var k = 0; k < cyy.length; k++)
					u_yy[3*i+j] += uyy[k][i][j]*ayy[k][A_PM]*byy[k][B_PM]*alyy[k][AL_PM]*tyy[k][T_PM]*cyy[k];
		}
	}
	...
	hx = math.mean(math.add(math.subset(u_xx,math.index(math.add(math.multiply(nodesYT,3),1))),YT))-math.mean(math.add(math.subset(u_xx,math.index(math.add(math.multiply(nodesYB,3),1))),YB));
	wy = math.mean(math.add(math.subset(u_yy,math.index(math.add(math.multiply(nodesXR,3),0))),XR))-math.mean(math.add(math.subset(u_yy,math.index(math.add(math.multiply(nodesXL,3),0))),XL));
	...
	for (var i = 0; i < a_sp.length; i++) {
		...
		for (var j = 0; j < al_sp.length; j++) {			
			...
			for (var u = 0; u < NUM_NODES; u++) {	
				for (var v = 0; v < 3; v++) {
					for (var k = 0; k < cxx.length; k++)
						upxx[3*u+v] += uxx[k][u][v]*axx[k][i_sp[i]]*bxx[k][B_PM]*alxx[k][j_sp[j]]*txx[k][T_PM]*cxx[k];
					
					for (var k = 0; k < cyy.length; k++)
						upyy[3*u+v] += uyy[k][u][v]*ayy[k][i_sp[i]]*byy[k][B_PM]*alyy[k][j_sp[j]]*tyy[k][T_PM]*cyy[k];
				}
			}
			...
			switch (slider.plot2cas) {
				case 'nu_xy':
					tracePt.z = [nus[0]];
					hx = math.mean(math.add(math.subset(upxx,math.index(math.add(math.multiply(nodesYT,3),1))),YT))-math.mean(math.add(math.subset(upxx,math.index(math.add(math.multiply(nodesYB,3),1))),YB));
					nusos[i].push(-10*(hx-h0)/h0);
					break;
					
				case 'nu_yx':
					tracePt.z = [nus[1]];
					wy = math.mean(math.add(math.subset(upyy,math.index(math.add(math.multiply(nodesXR,3),0))),XR))-math.mean(math.add(math.subset(upyy,math.index(math.add(math.multiply(nodesXL,3),0))),XL));
