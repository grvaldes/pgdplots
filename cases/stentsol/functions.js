	...
	for (var i = 0; i < NUM_NODES; i++) {
		for (var j = 0; j < deltx[i].length; j++) {
				...
				for (var k = 0; k < cxx.length; k++)
					deltx[i][j] += slider.defx*(uxx[k][i][j]*axx[k][A_PM]*bxx[k][B_PM]*alxx[k][AL_PM]*txx[k][T_PM]*cxx[k]);
				
				for (var k = 0; k < cyy.length; k++)
					delty[i][j] += slider.defy*(uyy[k][i][j]*ayy[k][A_PM]*byy[k][B_PM]*alyy[k][AL_PM]*tyy[k][T_PM]*cyy[k]);
		}
	}
...
function result() {
	...
	f4.add(slider, 'defy').min(0.00).max(1.00).step(0.01).name('R deformation').onChange(function() {remesh()});									// is remeshed in order to include the effects of the deformation. Deformations in here work in an
...
function calcNuVals() {		
	...
	w0 = (B_ARR[B_PM]-A_ARR[A_PM]*CAL_ARR[AL_PM])*nX;
	...
	for (var i = 0; i < NUM_NODES; i++) {
		for (var j = 0; j < deltx[i].length; j++) {
				...
				for (var k = 0; k < cxx.length; k++)
					u_xx[6*i+j] += uxx[k][i][j]*axx[k][A_PM]*bxx[k][B_PM]*alxx[k][AL_PM]*txx[k][T_PM]*cxx[k];
				
				for (var k = 0; k < cyy.length; k++)
					u_yy[6*i+j] += uyy[k][i][j]*ayy[k][A_PM]*byy[k][B_PM]*alyy[k][AL_PM]*tyy[k][T_PM]*cyy[k];
		}
	}
	...
	for (var i = 0; i < a_sp.length; i++) {
		...
		for (var j = 0; j < al_sp.length; j++) {
			...
			for (var u = 0; u < NUM_NODES; u++) {	
				for (var v = 0; v < 6; v++) {
					for (var k = 0; k < cxx.length; k++)
						upxx[6*u+v] += uxx[k][u][v]*axx[k][i_sp[i]]*bxx[k][B_PM]*alxx[k][j_sp[j]]*txx[k][T_PM]*cxx[k];
					
					for (var k = 0; k < cyy.length; k++)
						upyy[6*u+v] += uyy[k][u][v]*ayy[k][i_sp[i]]*byy[k][B_PM]*alyy[k][j_sp[j]]*tyy[k][T_PM]*cyy[k];
				}
			}
